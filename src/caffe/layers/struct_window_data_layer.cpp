...
StructWindowDataLayer<Dtype>::~StructWindowDataLayer<Dtype>() {
  this->JoinPrefetchThread();
}

template <typename Dtype>
void StructWindowDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  do {
    ...
    for (int i = 0; i < num_windows; ++i) {
      ...
      vector<float> window(StructWindowDataLayer::NUM);
      window[StructWindowDataLayer::IMAGE_INDEX] = image_index;
      window[StructWindowDataLayer::LABEL] = label;
      window[StructWindowDataLayer::OVERLAP] = overlap;
      window[StructWindowDataLayer::X1] = x1;
      window[StructWindowDataLayer::Y1] = y1;
      window[StructWindowDataLayer::X2] = x2;
      window[StructWindowDataLayer::Y2] = y2;

      // add window to foreground list or background list
      if (overlap >= fg_threshold) {
        int label = window[StructWindowDataLayer::LABEL];
        CHECK_GT(label, 0);
        ...
        window[StructWindowDataLayer::LABEL] = 0;
        window[StructWindowDataLayer::OVERLAP] = 0;
...
template <typename Dtype>
unsigned int StructWindowDataLayer<Dtype>::PrefetchRand() {
  CHECK(prefetch_rng_);
...
template <typename Dtype>
void StructWindowDataLayer<Dtype>::InternalThreadEntry() {
  ...
  for (int is_fg = 0; is_fg < 2; ++is_fg) {
    for (int dummy = 0; dummy < num_samples[is_fg]; ++dummy) {
      ...
      pair<std::string, vector<int> > image =
          image_database_[window[StructWindowDataLayer<Dtype>::IMAGE_INDEX]];

      cv::Mat cv_img;
      if (this->cache_images_) {
        pair<std::string, Datum> image_cached =
          image_database_cache_[window[StructWindowDataLayer<Dtype>::IMAGE_INDEX]];
      ...
      int x1 = window[StructWindowDataLayer<Dtype>::X1];
      int y1 = window[StructWindowDataLayer<Dtype>::Y1];
      int x2 = window[StructWindowDataLayer<Dtype>::X2];
      int y2 = window[StructWindowDataLayer<Dtype>::Y2];
      ...
      // get window label
      top_label[item_id] = window[StructWindowDataLayer<Dtype>::LABEL];
      ...
      inf << image.first << std::endl
          << window[StructWindowDataLayer<Dtype>::X1]+1 << std::endl
          << window[StructWindowDataLayer<Dtype>::Y1]+1 << std::endl
          << window[StructWindowDataLayer<Dtype>::X2]+1 << std::endl
          << window[StructWindowDataLayer<Dtype>::Y2]+1 << std::endl
          << do_mirror << std::endl
...
INSTANTIATE_CLASS(StructWindowDataLayer);
REGISTER_LAYER_CLASS(StructWindowData);
