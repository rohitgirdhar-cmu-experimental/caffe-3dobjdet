...
void ImageDataMultiLabelLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  label_shape[1] = lines_[0].second.size();
...
void ImageDataMultiLabelLayer<Dtype>::InternalThreadEntry() {
  ...
  for (int item_id = 0; item_id < batch_size; ++item_id) {
    ...
    for (int label_pos = 0; label_pos < lines_[lines_id_].second.size(); label_pos++) {
      prefetch_label[item_id * lines_[lines_id_].second.size() + label_pos] = lines_[lines_id_].second[label_pos];
    }
    // go to the next iter
    lines_id_++;
